...
#Reading JSON files about popular article sampling by period and categorized by kind
...
def article_overview(kind, period):
    ...
    for article in data:
        media = article['media']
        for media_items in media:
            media_item_metadata = media_items['media-metadata']
            for media_item_metadata in media_item_metadata:
                if (media_item_metadata ['format']=='Standard Thumbnail'):
                    urls.append(media_item_metadata ['url'])

    #urls = map(lambda sd: sd['url'])
    # YOUR CODE HERE
